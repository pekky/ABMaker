...
class ConservativeGPUAudioGenerator:
    """保守GPU利用率音频生成器"""
    ...
    def __init__(self, voice_preset: str = "v2/zh_speaker_1"):
        ...
        if torch.cuda.is_available():
            ...
            # 保守的CUDA设置
            torch.backends.cudnn.benchmark = False  # 关闭benchmark以减少GPU使用
            torch.backends.cuda.matmul.allow_tf32 = False  # 关闭TF32以减少GPU使用
            torch.backends.cudnn.allow_tf32 = False
            print("✓ 保守CUDA设置（减少GPU使用）")
            
            # 轻度预热GPU
            self._light_warmup_gpu()
            ...
            if (i + 1) % 4 == 0:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if gpu_util > 60:
                            print(f"⚠️ GPU利用率过高: {gpu_util}%，休息1秒...")
                            time.sleep(1)
                except:
                    pass
    ...
    def merge_audio_arrays(self, audio_arrays: List[np.ndarray], output_path: str, 
                          silence_duration: float = 0.05) -> str:  # 0.05秒静音
        """合并音频数组"""
        if not audio_arrays:
            return None
...
class ConservativeGPUAudiobookMaker:
    """保守GPU利用率有声读物制作器"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1",
                 max_chars_per_chunk: int = 300,  # 300字符片段
        ...
        self.audio_generator = ConservativeGPUAudioGenerator(
            voice_preset=voice_preset
        )
        
        # 状态文件
        self.state_file = "conservative_gpu_processing_state.json"
...
def main():
    parser = argparse.ArgumentParser(description="保守GPU利用率PDF转有声读物")
    parser.add_argument("pdf_path", help="PDF文件路径")
    parser.add_argument("-o", "--output_dir", default="conservative_gpu_audio", help="输出目录")
    parser.add_argument("-v", "--voice_preset", default="v2/en_speaker_6", help="语音预设")
    parser.add_argument("-c", "--max_chars_per_chunk", type=int, default=300, help="每片段最大字符数")
    ...
    maker = ConservativeGPUAudiobookMaker(
