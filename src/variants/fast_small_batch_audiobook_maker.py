...
class FastSmallBatchAudioGenerator:
    """快速小批次音频生成器，使用更快的模型配置"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1", 
                 max_workers: int = 6,  # 适中的并行数
                 batch_size: int = 12):  # 适中的批处理大小
        ...
        # 设置环境变量以使用更快的模型
        os.environ["SUNO_USE_SMALL_MODELS"] = "True"  # 使用小模型，速度更快
        os.environ["SUNO_OFFLOAD_CPU"] = "True"  # 启用CPU卸载，减少GPU压力
        
        print("⚡ 快速小批次模式启动")
        print(f"✓ 使用小模型（更快速度）")
        print(f"✓ 启用CPU卸载（减少GPU压力）")
    ...
    def _warmup_gpu(self):
        ...
        if torch.cuda.is_available():
            ...
            dummy_tensors = []
            try:
                for i in range(5):  # 减少预热张量数量
                    tensor = torch.randn(500, 500, device='cuda')  # 减小张量大小
    ...
    def generate_audio_batch(self, text_chunks: List[str]) -> List[np.ndarray]:
        ...
        for batch_idx in range(total_batches):
            ...
            end_idx = min(start_idx + self.batch_size, len(text_chunks))
            batch_chunks = text_chunks[start_idx:end_idx]
            
            print(f"  处理批次 {batch_idx + 1}/{total_batches} ({len(batch_chunks)} 个片段)")
            
            # 使用多线程并行处理
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [executor.submit(self.generate_single_audio, chunk) for chunk in batch_chunks]
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        print(f"❌ 批次处理错误: {e}")
                        # 添加静音作为fallback
                        audio_arrays.append(np.zeros(int(self.sample_rate * 0.5), dtype=np.float32))
...
class FastSmallBatchAudiobookMaker:
    """快速小批次有声读物制作器"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1",
                 max_chars_per_chunk: int = 150,  # 减小片段大小
                 target_tokens_per_batch: int = 10000,  # 1万token批次
                 max_workers: int = 6,
                 batch_size: int = 12,
                 resume: bool = True):
        self.voice_preset = voice_preset
        ...
        self.audio_generator = FastSmallBatchAudioGenerator(
        ...
        # 状态文件
        self.state_file = "fast_small_batch_processing_state.json"
...
def main():
    parser = argparse.ArgumentParser(description="快速小批次PDF转有声读物")
    parser.add_argument("pdf_path", help="PDF文件路径")
    parser.add_argument("-o", "--output_dir", default="fast_small_batch_audio", help="输出目录")
    ...
    parser.add_argument("-w", "--max_workers", type=int, default=6, help="最大并行工作线程数")
    parser.add_argument("-b", "--batch_size", type=int, default=12, help="批处理大小")
    ...
    maker = FastSmallBatchAudiobookMaker(
