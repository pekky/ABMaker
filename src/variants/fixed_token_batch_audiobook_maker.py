# -*- coding: utf-8 -*-
"""
修复版按4万token分批处理有声读物制作器 - 每批生成一个音频文件
修复CUDA多进程问题
"""
import os
import argparse
...
from core.audio_generator import AudioGenerator  # 使用原始的单线程版本


class FixedTokenBatchAudiobookMaker:
    """修复版按4万token分批处理有声读物制作器"""
    ...
    def __init__(self, voice_preset: str = "v2/zh_speaker_1", 
        ...
        # 使用单线程版本避免CUDA多进程问题
        self.audio_generator = AudioGenerator(voice_preset=voice_preset, use_small_model=False)
        
        # 状态文件路径
        self.state_file = "fixed_token_batch_processing_state.json"
        self.output_dir = "fixed_token_batch_audio_files"
    ...
    def create_audiobook_token_batch(self, pdf_path: str, output_dir: str = "fixed_token_batch_audio_files",
        ...
        """
        print("=" * 60)
        print(f"📚 开始修复版按4万token分批制作有声读物（每批一个音频文件）")
        print("=" * 60)
...
def main():
    """命令行入口"""
    parser = argparse.ArgumentParser(description="修复版按4万token分批PDF小说转有声读物工具（每批一个音频文件）")
    parser.add_argument("pdf_file", help="PDF文件路径")
    parser.add_argument("-o", "--output-dir", default="fixed_token_batch_audio_files", 
                       help="输出目录（默认: fixed_token_batch_audio_files）")
    ...
    # 创建修复版按token分批处理制作器
    maker = FixedTokenBatchAudiobookMaker(
