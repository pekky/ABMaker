...
class MemoryOptimizedAudioGenerator:
    """内存优化版音频生成器，充分利用GPU显存"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1", 
                 use_small_model: bool = False, 
                 max_workers: int = 8,  # 增加并行数
                 batch_size: int = 16):  # 增加批处理大小
        ...
        # 设置环境变量以充分利用显存
        os.environ["SUNO_USE_SMALL_MODELS"] = "False"  # 使用完整模型
        os.environ["SUNO_OFFLOAD_CPU"] = "False"  # 不卸载到CPU，充分利用GPU
        
        print("🚀 内存优化模式启动")
        print(f"✓ 使用完整模型（最高质量）")
        print(f"✓ 禁用CPU卸载（充分利用GPU显存）")
...
class MemoryOptimizedAudiobookMaker:
    """内存优化版有声读物制作器"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1",
                 max_chars_per_chunk: int = 200,
                 target_tokens_per_batch: int = 40000,
                 max_workers: int = 8,
                 batch_size: int = 16,
                 resume: bool = True):
        self.voice_preset = voice_preset
        ...
        self.audio_generator = MemoryOptimizedAudioGenerator(
        ...
        # 状态文件
        self.state_file = "memory_optimized_processing_state.json"
...
def main():
    parser = argparse.ArgumentParser(description="内存优化版PDF转有声读物")
    parser.add_argument("pdf_path", help="PDF文件路径")
    parser.add_argument("-o", "--output_dir", default="memory_optimized_audio", help="输出目录")
    ...
    parser.add_argument("-t", "--target_tokens_per_batch", type=int, default=40000, help="目标批次Token数")
    parser.add_argument("-w", "--max_workers", type=int, default=8, help="最大并行工作线程数")
    parser.add_argument("-b", "--batch_size", type=int, default=16, help="批处理大小")
    ...
    maker = MemoryOptimizedAudiobookMaker(
