...
class OptimizedGPUAudioGenerator:
    """优化GPU利用率音频生成器"""
    ...
    def merge_audio_arrays(self, audio_arrays: List[np.ndarray], output_path: str, 
                          silence_duration: float = 0.03) -> str:  # 0.03秒静音
        """合并音频数组"""
        if not audio_arrays:
            return None
...
class OptimizedGPUAudiobookMaker:
    """优化GPU利用率有声读物制作器"""
    
    def __init__(self, voice_preset: str = "v2/zh_speaker_1",
                 max_chars_per_chunk: int = 200,  # 200字符片段
        ...
        self.audio_generator = OptimizedGPUAudioGenerator(
            voice_preset=voice_preset
        )
        
        # 状态文件
        self.state_file = "optimized_gpu_processing_state.json"
...
def main():
    parser = argparse.ArgumentParser(description="优化GPU利用率PDF转有声读物")
    parser.add_argument("pdf_path", help="PDF文件路径")
    parser.add_argument("-o", "--output_dir", default="optimized_gpu_audio", help="输出目录")
    ...
    maker = OptimizedGPUAudiobookMaker(
