#!/bin/bash
# 智能批次处理启动脚本 - 4万字符+完整页面/段落

echo "🧠 启动智能批次PDF转有声读物处理（4万字符+完整页面/段落）"
echo "=========================================================="

# 检查参数
if [ $# -lt 1 ]; then
    echo "用法: $0 <PDF文件> [输出文件] [语音预设] [并行进程数]"
    echo "示例: $0 RivewTown.pdf smart_RiverTown.wav v2/en_speaker_6 2"
    exit 1
fi

PDF_FILE="$1"
OUTPUT_FILE="${2:-smart_audiobook.wav}"
...
echo "✓ 使用完整模型（最高音频质量）"
echo "✓ 使用全精度处理（最高音频质量）"
echo "✓ 智能批次处理（4万字符+完整页面/段落）"
echo ""

# 启动智能批次处理
echo "开始智能批次处理..."
python3 smart_batch_audiobook_maker.py \
    "$PDF_FILE" \
    -o "$OUTPUT_FILE" \
    -v "$VOICE_PRESET" \
    -c 200 \
