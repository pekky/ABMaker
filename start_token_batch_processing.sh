#!/bin/bash
# 按4万token分批处理启动脚本 - 每批生成一个音频文件

echo "📚 启动按4万token分批PDF转有声读物处理（每批一个音频文件）"
echo "=========================================================="

# 检查参数
if [ $# -lt 1 ]; then
    echo "用法: $0 <PDF文件> [输出目录] [语音预设] [并行进程数]"
    echo "示例: $0 RivewTown.pdf token_batch_audio v2/en_speaker_6 6"
    exit 1
fi

PDF_FILE="$1"
OUTPUT_DIR="${2:-token_batch_audio_files}"
...
# 启动按token分批处理
echo "开始按4万token分批处理..."
python3 token_batch_audiobook_maker.py \
    "$PDF_FILE" \
    -o "$OUTPUT_DIR" \
    -v "$VOICE_PRESET" \
    -c 200 \
